// Photo and ID config for trip styles — language-independent
// Replace placeholder paths with real photos in /public/photos/styles/
export const tripStyleMeta = [
  { id: "nature",       photo: "/photos/styles/nature.jpg",       emoji: "🦜" },
  { id: "culture",      photo: "/photos/styles/culture.jpg",      emoji: "🏛️" },
  { id: "adventure",    photo: "/photos/styles/adventure.jpg",    emoji: "⛰️" },
  { id: "food",         photo: "/photos/styles/food.jpg",         emoji: "☕" },
  { id: "beaches",      photo: "/photos/styles/beaches.jpg",      emoji: "🏝️" },
  { id: "wellness",     photo: "/photos/styles/wellness.jpg",     emoji: "🧘" },
  { id: "rest",         photo: "/photos/styles/rest.jpg",         emoji: "🌅" },
  { id: "solo_women",   photo: "/photos/styles/solo_women.jpg",   emoji: "🌺" },
  { id: "coffee",       photo: "/photos/styles/coffee.jpg",       emoji: "☕" },
  { id: "holistic",     photo: "/photos/styles/holistic.jpg",     emoji: "🌿" },
  { id: "honeymoon",    photo: "/photos/styles/honeymoon.jpg",    emoji: "🌹" },
  { id: "party",        photo: "/photos/styles/party.jpg",        emoji: "🎶" },
  { id: "photography",  photo: "/photos/styles/photography.jpg",  emoji: "📷" },
  { id: "birdwatching", photo: "/photos/styles/birdwatching.jpg", emoji: "🦅" },
  { id: "surf",         photo: "/photos/styles/surf.jpg",         emoji: "🏄" },
  { id: "gastronomy",   photo: "/photos/styles/gastronomy.jpg",   emoji: "🍽️" },
];
